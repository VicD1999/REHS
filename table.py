...
distances = [150, 400, 2000] # km

dico = {**{"Battery pack": (BATTERY_LIFETIME, f"{round(BATTERY_PRICE/1000)} \$/kWh", -1, -1),
           "UFWT": (TURBINE_LIFETIME, f"{round(TURBINE_PRICE)} \$/MW", -1, -1)},
        **{f"Boat ({boat_opti_mass(d):.0f} tons)": (BOAT_LIFETIME, f"{round(boat_CAPEX(boat_opti_mass(d)))} \$/ton", BOAT_VELOCITY, losses_boat(1000, boat_opti_mass(d))) 
           for d in distances},
        "Airship (4400 tons)": (AIRSHIP_LIFETIME, f"{round(AIRSHIP_PRICE / 4400)} \$/ton" , AIRSHIP_VELOCITY_L, losses_airship(1000, 4400)),
